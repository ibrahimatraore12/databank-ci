...
from components.ui import (  # noqa: E402
    afficher_barre_score, afficher_entete, afficher_pied_de_page, label_technique, requete_duckdb, t,
)
...
with col1, st.container(border=True):
    st.metric(t("client_forte_valeur_a_risque"), int(df["is_high_value_at_risk"].sum()))
with col2, st.container(border=True):
    st.metric(t("risque_reclamation"), int(df["is_complaints_churn_risk"].sum()))
with col3, st.container(border=True):
    st.metric(t("salaire_domicilie_digital_dormant"), int(df["is_digitally_dormant_salary"].sum()))
